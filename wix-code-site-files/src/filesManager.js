...
module.exports = (basePath) => {
    ...
    const get = (currentPath = basePath) => {
        ...
        if(fs.lstatSync(currentPath).isDirectory()){
            fs.readdirSync(currentPath).forEach((itemName) => 
                result = Object.assign({}, result, get(path.resolve(currentPath, itemName)))
            )
        } else {
            result = Object.assign({}, result, {[path.relative(basePath, currentPath)]: fs.readFileSync(currentPath, 'utf8')})
        }
        return result
    }
    return {
